...
async function run() {
    try {
        ...
        const logsRes = await client.query('SELECT event_type, user_name, description, timestamp FROM audit_logs WHERE invoice_id = $1 ORDER BY timestamp DESC', ['8a050732-e77d-4e1b-8dbf-d6e82f47c109']);
        
        console.log('--- FINAL REPORT: 8a050732-e77d-4e1b-8dbf-d6e82f47c109 ---');
        if (invRes.rows.length > 0) {
            ...
            console.log('JSON Key "Supplier GST":', raw['Supplier GST']);
            console.log('JSON Key "vendor_gst":', raw['vendor_gst']);
        } else {
