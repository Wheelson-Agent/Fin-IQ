 ...
 *                           Only files added AFTER the watcher starts are processed.
...
function startWatcher(companyId: string, folderPath: string): void {
    ...
    const watcher = chokidar.watch(folderPath, {
        ...
        persistent: true,
        ignoreInitial: true,     // only files added AFTER watcher starts are processed
        awaitWriteFinish: {
...
async function loadAndApplyWatchers(): Promise<void> {
    ...
    for (const company of companies) {
        // Read from source_config (dedicated key) first, fall back to full_config
        ...
        if (!sourceConfig) {
            ...
            sourceConfig = fullConfig ? { sources: fullConfig.sources, sourceConfigs: fullConfig.sourceConfigs } : null;
        }

        const enabled = sourceConfig?.sources?.local_folder === true;
        const folderPath = (sourceConfig?.sourceConfigs?.local_folder?.folderPath || '').trim();
...
/**
 * Called from ipc.ts after config save so toggle changes take effect
