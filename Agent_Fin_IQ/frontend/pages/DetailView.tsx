            ...
            <div className="flex items-center gap-2 border-l border-slate-200 pl-4 ml-2">
              ...
              {isVendorMapped && (
                <Button
                  ...
                  disabled={saving}
                  onClick={async () => {
                    // Existing approval/re-validate logic
                    if (isHandoff || invoice.status === 'Awaiting Input') {
                       console.log('Re-validating bill:', id);
                       // We can call runPipeline here if needed for revalidation
                    } else {
                       console.log('Approving bill:', id);
                    }
                  }}
                >
                  {saving ? <RefreshCw size={20} className="animate-spin" /> : ((isHandoff || invoice.status === 'Awaiting Input') ? <RefreshCcw size={20} /> : <CheckCircle size={20} />)}
                </Button>
              )}
            </div>
          )}
